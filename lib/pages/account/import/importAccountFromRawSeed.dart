...
class _ImportAccountFromRawSeedState extends State<ImportAccountFromRawSeed> {
  ...
  Future<void> _refreshAccountAddress(String v) async {
    ...
    if (widget.service.plugin.basic.pluginType == PluginType.Etherem) {
      //privateKey
      if (value.length == 66 || value.length == 64) {
        //0x
        final addressInfo = await widget.service.plugin.sdk.api.ethKeyring
            .addressFromPrivateKey(privateKey: value);
        setState(() {
          _addressIcon = addressInfo;
        });
      }
    } else if (value.length <= 32 || value.length == 66) {
