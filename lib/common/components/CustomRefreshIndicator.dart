...
enum CustomRefreshIndicatorMode {
...
class CustomRefreshIndicatorState extends State<CustomRefreshIndicator>
  ...
  CustomRefreshIndicatorMode _mode;
  ...
  bool _handleScrollNotification(ScrollNotification notification) {
    ...
    if (_shouldStart(notification)) {
      setState(() {
        _mode = CustomRefreshIndicatorMode.drag;
      });
      return false;
    }
    ...
    if (indicatorAtTopNow != _isIndicatorAtTop) {
      if (_mode == CustomRefreshIndicatorMode.drag ||
          _mode == CustomRefreshIndicatorMode.armed)
        dismiss(CustomRefreshIndicatorMode.canceled);
      ...
      if (_mode == CustomRefreshIndicatorMode.armed &&
          notification.dragDetails == null) {
      ...
      switch (_mode) {
        case CustomRefreshIndicatorMode.armed:
          _show();
          break;
        case CustomRefreshIndicatorMode.drag:
          dismiss(CustomRefreshIndicatorMode.canceled);
          break;
        default:
          // do nothing
  ...
  void _checkDragOffset(double containerExtent) {
    assert(_mode == CustomRefreshIndicatorMode.drag ||
        _mode == CustomRefreshIndicatorMode.armed);
    double newValue =
  ...
  Future<void> dismiss(CustomRefreshIndicatorMode newMode) async {
    await Future<void>.value();
    // This can only be called from _show() when refreshing and
    // _handleScrollNotification in response to a ScrollEndNotification or
    // direction change.
    assert(newMode == CustomRefreshIndicatorMode.canceled ||
        newMode == CustomRefreshIndicatorMode.done);
    setState(() {
      _mode = newMode;
    });
    switch (_mode) {
      case CustomRefreshIndicatorMode.done:
        await _scaleController.animateTo(1.0,
            duration: _kIndicatorScaleDuration);
        break;
      case CustomRefreshIndicatorMode.canceled:
  ...
  void _show() {
    assert(_mode != CustomRefreshIndicatorMode.refresh);
    assert(_mode != CustomRefreshIndicatorMode.snap);
    final Completer<void> completer = Completer<void>();
    _pendingRefreshFuture = completer.future;
    _mode = CustomRefreshIndicatorMode.snap;
    _positionController
        .animateTo(1.0 / _kDragSizeFactorLimit,
            duration: _kIndicatorSnapDuration)
        .then<void>((void value) {
      if (mounted && _mode == CustomRefreshIndicatorMode.snap) {
        assert(widget.onRefresh != null);
        setState(() {
          // Show the indeterminate progress indicator.
          _mode = CustomRefreshIndicatorMode.refresh;
        });
        ...
        refreshResult.whenComplete(() {
          if (mounted && _mode == CustomRefreshIndicatorMode.refresh) {
            completer.complete();
            dismiss(CustomRefreshIndicatorMode.done);
          }
        });
      }
    });
  ...
  Future<void> show({bool atTop = true}) {
    if (_mode != CustomRefreshIndicatorMode.refresh &&
        _mode != CustomRefreshIndicatorMode.snap) {
  ...
  Widget build(BuildContext context) {
    ...
    final bool showIndeterminateIndicator =
        _mode == CustomRefreshIndicatorMode.refresh ||
            _mode == CustomRefreshIndicatorMode.done;
